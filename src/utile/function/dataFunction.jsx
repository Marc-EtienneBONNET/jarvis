...
async function takeEvents(events){
    let tmp = ((await axios.get('http://localhost:3001/event/getTakeAll')).data).map((element) => {
                return ({
                    ...element,
...
export async function dataRefrech(arg){
    ...
    let profiles;
    if (arg.events)
        events = await takeEvents(arg.events.events);
    if (arg.profiles)
        profiles = await takeProfiles(arg.profiles.profiles);
    if (events !== undefined || profiles !== undefined)
    {
        if (arg.events && events)
            setTimeout(() => {arg.events.setEvents(events)}, 1000);
        if (arg.profiles && profiles)
            setTimeout(() => {arg.profiles.setProfile(profiles)}, 1000);
    }
    else
        setTimeout(() => {dataRefrech(arg)}, 1000); 
}
